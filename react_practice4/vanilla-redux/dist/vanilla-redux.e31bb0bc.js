...
var btnDecrease = document.querySelector('#decrease'); //액션 함수 정의
//1.의미: 프로젝트의 상태에 변화를 일으키는 것을 액션이라고 한다.
//2.액션 이름은 문자열 형태로 대문자로 작성한다.

var TOGGLE_SWITCH = 'TOGGLE_SWITCH';
var INCREASE = 'INCREASE';
var DECREASE = 'DECREASE'; //이 액션 이름을 사용하여 액션 객체를 만드는
//액션 생성 함수를 작성한다.
...
render(); //store 상태가 바뀔 때 마다 render 함수가 호출되도록 해 줄 것.
//이 작업은 내장함수 subscribe를 사용한다.
//subscribe 함수의 파라미터는 함수 형태의 값을 전달해준다.
//이렇게 전달된 함수는 추후 액션이 발생하여
//상태가 업데이트 될 때마다 호출된다.

store.subscribe(render); //액션을 발생시키는 것을 디스패치라고 한다.
